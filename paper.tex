...
third parties~\cite{vcnn2024}. TEE attestation proves that \emph{a specific
...
the deterministic EVM execution trace after a block is finalized.
\end{definition}
...
contract, anchoring $H_t$ to block $b$ on-chain.
...
\mathbb{G}_1 \times \mathbb{G}_2 \times \mathbb{G}_1$ over BN254, certifying:
\begin{equation}
...
with current hardware~\cite{peng2025survey,chen2024zkml}. ZKP-RA exploits the
...
\paragraph{Optimistic agent governance.}
Dispute-based systems borrow the optimistic rollup model to govern autonomous
agents by challenging suspected malicious transactions during a window period.
Such systems are vulnerable to well-timed attacks that settle before a challenge
completes. ZKP-RA provides cryptographic pre-settlement guarantees that do not
depend on active monitoring or economic stake during a challenge window.
...
Future work will investigate batch-proof aggregation using Plonky2 or HyperPlonk
to amortize the per-transaction proving overhead across agent epochs, and will
examine the integration of ZKP-RA with TEE attestation chains to provide
defense-in-depth against hardware-level adversaries.
...
Chen, B.J., Waiwitlikhit, S., Stoica, I., \& Kang, D. (2024).
ZKML: An Optimizing System for ML Inference in Zero-Knowledge Proofs.
In \textit{Proceedings of the 2024 European Conference on Computer Systems
(EuroSys '24)}, pp.\ 560--574. Association for Computing Machinery.
\newblock \doi{10.1145/3627703.3650088}

\bibitem{peng2025survey}
Peng, Z., Zhao, C., Wang, T., Liao, G., Lin, Z., Liu, Y., Cao, B., Shi, L.,
Yang, Q., \& Zhang, S. (2025).
A Survey of Zero-Knowledge Proof Based Verifiable Machine Learning.
\textit{Artificial Intelligence Review}.
\newblock \doi{10.48550/arXiv.2502.18535}

\bibitem{verillm2025}
VeriLLM: A Lightweight Framework for Publicly Verifiable Decentralized Inference.
(2025). \textit{arXiv preprint arXiv:2509.24257v4}.

\bibitem{vcnn2024}
vCNN: Verifiable Convolutional Neural Network Based on zk-SNARKs.
(2024). \textit{IEEE Transactions on Dependable and Secure Computing}, 21(4).
